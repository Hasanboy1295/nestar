...
import { MemberStatus, MemberType } from 'apps/nestar-api/src/libs/member.enum';
...
export class BatchService {
	...
	public async batchTopProperties(): Promise<void> {
		...
		const promisedList = properties.map(async (ele: Property) => {
			...
			const rank = propertyLikes * 2 + propertyViews * 1;//qonunyat like va viewdan kelib ciqgan. holda  rankni tawkil etayabmiz 
			return await this.propertyModel.findByIdAndUpdate(_id, { propertyRank: rank }); //property rankni ozgartryabmiz
		});
